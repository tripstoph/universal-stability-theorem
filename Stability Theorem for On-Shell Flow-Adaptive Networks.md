...
Edge weights are adaptive: they move under a force that balances a constitutive restoring term \(V'(w_e)\) against the squared on-shell drop. The coupling \(c\ge 0\) is a parameter of this model, not a symbol from the Kirchhoff–Thomson literature [@kirchhoff1847; @doyle1984]. The on-shell force \(F_c\) and energy \(\Phi\) are defined by
\[
F_c(w)_e=\frac{c}{2}(\Delta\Psi_e(w))^2+V'(w_e),
\qquad
\Phi(w)=\sum_{e\in E}V(w_e)-\frac{c}{2}\,\eta^T M_w^{-1}\eta.
\]
An on-shell equilibrium is a point \(w^\star\in\mathbb{R}_{>0}^E\) with \(F_c(w^\star)=0\). Lemma 2 records \(F_c=\nabla\Phi\). Write \(\Omega:=\mathbb{R}_{>0}^E\).
...
A two-vertex, one-edge path (`UniversalStability.Examples.TwoNodeExample`) inhabits the hypotheses of Theorem 5 at \(c=12\) and \(w^\star=1\). Kirchhoff current is \(1\), so \(\Delta\Psi=1/w^\star=1\), and the coupling term \(\frac{c}{2}(\Delta\Psi)^2=6\) cancels \(V'(1)=-6\). Instantiating `theorem5_local_invariance` produces an \(r_0>0\), so the hypotheses do not imply `False`. (The theorems themselves are for every \(c\ge 0\); \(c=0\) is the uncoupled constitutive special case, and is not the inhabiting instance.)
